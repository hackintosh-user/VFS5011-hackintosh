...
static void print_menu(void) {
    ...
    printf("  * Sensor Model         : VFS5011 PID {0x%04X}\n", VFS5011_PID);
    printf("%s%s%s\n\n", VFSC_CYAN, VFSC_RULE, VFSC_RESET);
}
 ...
 * vfs_client is already running as root at this point (see main()'s
...
static void do_deploy(void) {
    ...
    printf("\n%sDeploying Authentication Service...%s\n", VFSC_CYAN, VFSC_RESET);

    char cmd[PATH_MAX + 32];
    snprintf(cmd, sizeof(cmd), "sh \"%s/%s\" 2>&1", g_exec_dir, AGENT_INSTALL_SCRIPT_NAME);
