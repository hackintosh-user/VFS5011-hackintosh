#!/bin/bash
#
# hack-touchid-store-password.sh
#
# Stores the macOS login password inside the encrypted HackTouchIDStore volume,
...
# password file itself, because HackTouchIDStore already provides that.
#
# Run this whenever you need to set/update the stored password:
#   sudo ./hack-touchid-store-password.sh
#
...
if [ "$(id -u)" -ne 0 ]; then
    echo "Must run as root (sudo ./hack-touchid-store-password.sh)." >&2
    exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "== Mounting HackTouchIDStore =="
"$SCRIPT_DIR/hack-touchid-volume-mount.sh" >/dev/null
...
if [ "$PASSWORD" != "$PASSWORD_CONFIRM" ]; then
    ...
    "$SCRIPT_DIR/hack-touchid-volume-unmount.sh" >/dev/null
    exit 1
fi
...
echo "== Unmounting HackTouchIDStore =="
"$SCRIPT_DIR/hack-touchid-volume-unmount.sh" >/dev/null
