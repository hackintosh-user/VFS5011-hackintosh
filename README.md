...
<p align="center">
  ...
  * Please note that this branch is currently stale, the new major release being developed is at the ```active-development``` branch, this branch (```main```) will only get updated once the PR is merged but that is currently unknown still due to the amount of remaining work still.
</p>
