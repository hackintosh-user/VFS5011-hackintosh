#!/bin/bash
#
# vfs5011_volume_mount.sh
#
# Mounts the encrypted VFSStore volume on demand, pulling its passphrase
...
# with vfs5011_volume_unmount.sh right after.
#
# Must run as root (the keychain item and the volume itself are both
# root-only).
#
# Usage: ./vfs5011_volume_mount.sh
# On success, prints the mount point path on stdout (last line) so a
# caller can capture it: MOUNT_PATH=$(./vfs5011_volume_mount.sh | tail -1)
