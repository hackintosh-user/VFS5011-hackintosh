#!/bin/bash
#
# hack-touchid-agent-install.sh (v2)
#
...
#
#   sudo ./hack-touchid-agent-install.sh
#
set -e
...
# a bare `sudo ./hack-touchid-agent-install.sh` from the project folder
...
# The daemon resolves hack-touchid-volume-mount.sh / _unmount.sh relative to
...
# episode fails at the very first step ("sh: .../hack-touchid-volume-mount.sh:
...
# single Deploy, not just once. See hack-touchid-grant-accessibility.sh for
# why this became necessary (a NULL-csreq grant that worked fine on
# Sequoia silently did nothing on Tahoe).
GRANT_SCRIPT="$SCRIPT_DIR/hack-touchid-grant-accessibility.sh"
...
# console-owner lookup for a bare `sudo ./hack-touchid-agent-install.sh`.
...
# hack-touchid-agent-install.sh -- safe to delete if this project is removed.
