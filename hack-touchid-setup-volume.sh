...
#   sudo ./hack-touchid-setup-volume.sh
#
...
if [ "$(id -u)" -ne 0 ]; then
    echo "This must be run as root (sudo ./hack-touchid-setup-volume.sh)." >&2
    exit 1
fi
...
echo "reboot. Use hack-touchid-volume-mount.sh to mount it on demand, and"
echo "hack-touchid-volume-unmount.sh to unmount it after use."
